...
if (!empty($_FILES) && $_POST['token'] == $verifyToken) {
	...
	if (in_array(strtolower($fileParts['extension']),$fileTypes)) {
		...
		$width_ss=intval($ym_idsort[intval($c_toid)]['width'])==0?200:intval($ym_idsort[intval($c_toid)]['width']);
		$height_ss=intval($ym_idsort[intval($c_toid)]['height'])==0?160:intval($ym_idsort[intval($c_toid)]['height']);


		$img->param($targetFile)->thumb($smallfile,$width_ss,$height_ss,intval($smalltype));

		//$img->param($targetFile)->thumb($smallfile, $c_uw,$c_uh,intval($smalltype));
		...
		$db->insert(''.$stype.'_pic', array('c_id' => intval($login_id),'c_sid' => intval($site_id),'c_toid' => intval($c_toid),'c_time' => time(), 'c_simg' => ptitle($smallfilename),'c_bimg' => ptitle($newfilename), 'c_title' => ptitle($titlename)));
		$lastid=$db->lastinsertid();
		$tempcode12252 = jcode($lastid);
		$db->update(''.$stype.'_pic', array('c_code' =>$tempcode12252), array('id' => $lastid));
