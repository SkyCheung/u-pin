...
class AcpService {
	...
	static function sign(&$params, $cert_path=SDK_SIGN_CERT_PATH, $cert_pwd=SDK_SIGN_CERT_PWD) {
		...
		if ($sign_falg) {
			...
			$logs( "银联：orderId=".$params ['orderId']." queryId=".$params ['orderId'].">>>>>签名失败<<<<<<<",'pay');
		}
	}
...
//	echo date('Y-m-d',time());
	...
	static function createAutoFormHtml($params, $reqUrl, $is_autosubmit =false) {
      	$html .='<form id="pay_form" name="pay_form" action="'.$reqUrl.'" target="_blank" method="post">';
		foreach ( $params as $key => $value ) {
			$html .= " <input type='hidden' name='".$key."' id='".$key."' value='".$value."' />";
		}
		
		//submit按钮控件请不要含有name属性
        $html .= "<input type='submit' class='sub btn' value='立即支付'></form>";		
		if($is_autosubmit ==1)
		{
