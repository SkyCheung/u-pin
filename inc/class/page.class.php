...
class Core_Lib_Page
{
	...
	protected function _get_link($page,$text)
	{
		switch ($this->method) {
			...
			case 'html':
				...
				return '<a href="' .$url . '" class="pagenum">' . $text . '</a>' . "\n";
			break;
	...
	protected function _get_link2($page,$text)
	{
		switch ($this->method) {
			case 'ajax':
				$parameter = '';
    ...
    public function last_page($name = '最后一页')
    {
 		...
 		{
 			return $this->_get_link2($this->total_pages, $name);
 		}	
 		return '';
    }  
    ...
    public function up_page($name = '上一页')
    {
    	if($this->now_page != 1)
    	{
    		return $this->_get_link2($this->now_page - 1, $name);
    	}else{
			return $this->_get_link2($this->now_page, $name);
		}
    	return '';
    }
    ...
    public function show($param = 1)
    {
		...
		$list = array('pages'=>'','first_page'=>'','last_page'=>'');
    	if(in_array($className, $classNames))
    	{			
			$list['pages']= $this->$className();
			$list['first_page']= $this->first_page();
			$list['last_page']= $this->last_page(); 
    		return $list;
    	}
  		return $list;
    }
    
    protected function show_0()
    {
