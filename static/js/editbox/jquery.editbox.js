...
(function($){
	$.fn.editBox = function(B) {
		...
		var B = $.extend({
			...
			maxX:0,                 //最大X座标初始化
			maxY:0,                 //最大Y座标初始化
			minHeight:600,            //最大高度限制初始化
			minWidth:1000,             //最大宽度限制初始化
		...
		var addCanvas = function(){
			...
			arr.push("<img src='"+ B.imgurl +"ico_arrow.png' class='arrow'/><ol></ol></ul>");
			//arr.push("<ul id='panelTemplate' style='clear:both;display:none;'><li>快递：</li><span>"+ B.deliveryName +"</span>")
			//arr.push("<img src='"+ B.imgurl +"ico_arrow.png' class='arrow'/><ol></ol></ul>");
			arr.push("<ul id='panelColor'><li>控件颜色:</li><a></a>")
			arr.push("<img src='"+ B.imgurl +"ico_arrow.png' class='arrow'/><ol></ol></ul>");
			arr.push("<ul id='panelPage'><li>页面设置</li><img src='"+ B.imgurl +"ico_arrow.png' class='arrow'/><ol></ol></ul>");
		...
		$.fn.content = function() {
			...
			_text = '{"width":'+ ($("#width")? $.trim($("#width").val()) : B.width)  +',"height":'+ ($("#height")? $.trim($("#height").val()) : B.height) +',"background":"'+ B.background +'","list":['+ _tempText.join(',') +']}';
			return _text;
		}
